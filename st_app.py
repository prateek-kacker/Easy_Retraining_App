'''
This is a streamlit which is ported from gradio app with file app.py. 
This app for uploading excel file, annotating the data, training the model and verifying the output for the model. 
You can keep repeating it till the model is getting desired results.
'''
import os
...
def moving_direction(direction, annotation_type):
    ...
    print(
        f'DF_INDEX {DF_INDEX}, index_index {index_index}, NEXT {NEXT}, PREVIOUS {PREVIOUS}')
...
def get_data_from_excel(excel_file):
    ...
    global base_df
    global MAX_LENGTH
    base_df = pd.read_excel(excel_file)
    base_df['Annotated'] = False
    base_df['Predicted'] = False
...
def train_fn(epochs,
    ...
    training_df['Label'] = training_df['Label'].map(
        lambda x: TEXT_CATEGORIES.index(x))
    ...
    tokenized_predict_datasets = predict_dataset.map(
        tokenize_function, batched=True)
    predict_df['Label'] = list(map(lambda x: TEXT_CATEGORIES[x],  trainer.predict(
        tokenized_predict_datasets).predictions.argmax(axis=1).tolist()))
...
uploaded_file = st.file_uploader("Get Data from excel")
if uploaded_file is not None:
    show_df_text =  get_data_from_excel(uploaded_file)
    st.write(show_df_text)
...
#     gr.Textbox(value=logging_fn, output=[], every=1,
#                label='Log Files', interactive=True)
